...
//		document.setProperty("com.genesyslab.externalevents.queue", "false");
	...
	protected IDocument renderInitialDocument(ILinkFactory links, InitialCommand initialCommand) {
		...
		userDataScript
				.appendText("\tgvpUserData = gvpUserData + key + '=' + session.com.genesyslab.userdata[key];\r\n");
		userDataScript.appendText("}\r\n");
		...
		for (int i = 0; i < parameterNames.length; ++i)
			nextLink.setParameters(parameterNames[i], initialCommand.getParameterValues(parameterNames[i]));
		...
		for (int i = 0; i < parameterNames.length; ++i)
			hangupLink.setParameters(parameterNames[i], initialCommand.getParameterValues(parameterNames[i]));
		...
		for (String event : events) {
			...
			for (int i = 0; i < parameterNames.length; ++i)
				eventLink.setParameters(parameterNames[i], initialCommand.getParameterValues(parameterNames[i]));
	...
	protected IDocument renderMetaDataRequest(ILinkFactory links, MetaDataRequestCommand metaDataMessageRequest) {
		...
		ILink hangupLink = links.createNextLink();
		for (int i = 0; i < parameterNames.length; ++i)
			hangupLink.setParameters(parameterNames[i], metaDataMessageRequest.getParameterValues(parameterNames[i]));
		hangupLink.setParameter(metaDataMessageRequest.getResultName(), metaDataMessageRequest.getHangupResultValue());
		Catch disconnectCatch = new Catch("connection.disconnect.hangup");
		disconnectCatch.addAction(new Goto(hangupLink.toString()));
		form.addEventHandler(disconnectCatch);
...
//		send.setNameList(nameList.toString());
		...
		form.addFormElement(block);
		ILink hangupLink = links.createNextLink();
		hangupLink.setParameter(metaDataMessageCommand.getResultName(), metaDataMessageCommand.getHangupResultValue());
		Catch disconnectCatch = new Catch("connection.disconnect.hangup");
		disconnectCatch.addAction(new Goto(hangupLink.toString()));
		form.addEventHandler(disconnectCatch);
		return this.createVXMLDocument(links, form);
	...
	@Override
	public Map<String, String> processMetaDataResponse(MetaDataConfiguration configuration, IActionContext context) {
