...
public class CvpPlatform extends VoicePlatform {
	...
	protected IDocument renderInputRequest(ILinkFactory links, InputRequestCommand inputRequestCommand) {
		...
		if ("dtmf only".equalsIgnoreCase(inputMode)) //$NON-NLS-1$
		{
			...
			try {
				if (sensitivity != null && sensitivity.length() > 0)
					field.setProperty(NAME_SENSITIVITY, new BigDecimal(sensitivity).divide(ONE_HUNDRED).toString());
			} catch (NumberFormatException e) {
				e.printStackTrace();
			}
			try {
			...
			if (speechIncompleteTimeout != null)
				field.setProperty(NAME_INCOMPLETETIMEOUT, speechIncompleteTimeout.toTimeString(TimeValue.SECONDS)); // $NON-NLS-1$
				...
				if ("dtmf only".equalsIgnoreCase(inputMode)) {
					...
					if (path.startsWith("dtmf:")) {
						field.addGrammar(new ExternalGrammar("dtmf",
								links.createResourceLink(inputRequestCommand.getInputValue()).toString()));
					} else {
						String query = "";
						...
						if (lastExtensionIndex != -1) {
							...
							IMediaProvider provider = mediaProviderRegistry
									.getMediaProvider(input.getProperty("media-provider"));
							ResourceGroup manager = (ResourceGroup) provider.getResourceManager();
							URL url = manager.getResource(path + lastPart + query);
		...
		if (inputRequestCommand.getInput2Value() != null) {
			...
			case InputRequestCommand.INPUT_TYPE_FILE:
				field.addGrammar(new ExternalGrammar("voice",
						links.createResourceLink(inputRequestCommand.getInput2Value()).toString()));
				break;
			case InputRequestCommand.INPUT_TYPE_CUSTOM:
		...
		filled.addVariable(new Variable(inputRequestCommand.getResultName(),
...
//				inputRequestCommand.getNoInputResultValue());
		...
		noInput.addVariable(new Variable(inputRequestCommand.getResultName(),
				"'" + inputRequestCommand.getNoInputResultValue() + "'"));
...
//				inputRequestCommand.getNoMatchResultValue());
		...
		noMatch.addVariable(new Variable(inputRequestCommand.getResultName(),
				"'" + inputRequestCommand.getNoMatchResultValue() + "'"));
...
//				"		lastresult = lastresult + '</lastresult>';\r\n");
		...
		for (int i = 0; i < parameterNames.length; ++i)
			hangupLink.setParameters(parameterNames[i], inputRequestCommand.getParameterValues(parameterNames[i]));
	...
	protected IDocument renderSelectionRequest(ILinkFactory links, SelectionRequestCommand selectionRequestCommand) {
		...
		TimeValue terminationTimeout = resolveTimeValue("termination-timeout",
				selectionRequestCommand.getPropertyValue("termination-timeout")); //$NON-NLS-1$
		String terminationCharacter = selectionRequestCommand.getPropertyValue("termination-character"); //$NON-NLS-1$
...
//		Menu menu = new Menu(selectionRequestCommand.getSelectionName(), prompt);
		...
		for (int i = 0; i < parameterNames.length; ++i)
			nextLink.setParameters(parameterNames[i], selectionRequestCommand.getParameterValues(parameterNames[i]));
		...
		for (int i = 0; i < selectionRequestCommand.getOptionCount(); ++i) {
			nextLink.setParameter(selectionRequestCommand.getSelectionName(), selectionRequestCommand.getOption(i));
		...
		ILink hangupLink = links.createNextLink();
		for (int i = 0; i < parameterNames.length; ++i)
			hangupLink.setParameters(parameterNames[i], selectionRequestCommand.getParameterValues(parameterNames[i]));
