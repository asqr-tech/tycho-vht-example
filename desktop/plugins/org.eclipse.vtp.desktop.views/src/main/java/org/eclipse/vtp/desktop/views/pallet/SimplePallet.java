...
public class SimplePallet implements Pallet {
	 ...
	 * @author Lonnie Pryor
	 */
	private final class ProviderSection extends LabelProvider implements IStructuredContentProvider,
			ITableLabelProvider, IMenuListener, DragSourceListener, IExpansionListener, PalletItemObserver {
		...
		Control initialize() {
			...
			viewer.addDragSupport(DND.DROP_COPY | DND.DROP_MOVE, new Transfer[] { PalletItemTransfer.getInstance() },
					this);
		...
		public void menuAboutToShow(IMenuManager manager) {
			provider.createMenu(null, manager,
					(PalletItem[]) ((IStructuredSelection) viewer.getSelection()).toList().toArray(new PalletItem[0]));
		}
		...
		public void dragStart(DragSourceEvent event) {
			...
			PalletItemTransfer.getInstance()
					.setPalletItem(((PalletItem) ((IStructuredSelection) viewer.getSelection()).getFirstElement()));
		}
