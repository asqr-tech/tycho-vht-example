...
public class BranchComponentPropertiesPanelProvider implements ComponentPropertiesPanelProvider
{
	public BranchComponentPropertiesPanelProvider()
	{
	}
		...
		IDesignComponent designComponent)
	{
		...
		ret.add(new BranchPropertiesPanel("Branch", pe));
		return ret;
	}
	...
	public boolean isApplicableFor(IDesignComponent designComponent)
	{
		if(!(designComponent instanceof PrimitiveElement))
			return false;
		PrimitiveElement pe = (PrimitiveElement)designComponent;
		return pe.getSubTypeId().equals("org.eclipse.vtp.modules.standard.ui.branch");
	}
