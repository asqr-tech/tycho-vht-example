		...
		for(TransferData type : aDropTargetEvent.dataTypes)
		{
			if(LocalSelectionTransfer.getTransfer().isSupportedType(type))
			{
				...
				try
				{
					...
					operation.copyResources(toCopy.toArray(new IResource[toCopy.size()]), designItemContainer.getUnderlyingFolder());
				}
				catch (Exception e)
				{
					e.printStackTrace();
		...
		TransferData transferType)
	{
		...
		if(LocalSelectionTransfer.getTransfer().isSupportedType(transferType))
		{
			...
			for(Object obj : selection.toList())
			{
				...
				{
					if(((IDesignDocument)obj).getParentDesignContainer().equals(designItemContainer)) //same parent
					{
						return Status.CANCEL_STATUS;
					}
				}
				...
				{
					if(((IDesignFolder)obj).getParentDesignContainer().equals(designItemContainer)) //same parent
					{
						return Status.CANCEL_STATUS;
					}
				}
