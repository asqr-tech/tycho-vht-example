...
public class DatabaseQueryDataMappingPropertiesPanel extends DesignElementPropertiesPanel
				...
				if (currentObject != null) {
					if (currentObject.getType().hasBaseType()) {
						if (!currentObject.getType().isObject()) {
							...
							List<IBusinessObject> businessObjects = getElement().getDesign().getDocument().getProject()
	...
	public void targetVariableChanged() {
		...
		if ((currentObject == null) || (!settings.targetVariableName.equals(currentObject.getPath())
	...
	public class MappingLabelProvider implements ITableLabelProvider {
		public Image getColumnImage(Object element, int columnIndex) {
			...
			if (columnIndex == 0) {
				return org.eclipse.vtp.desktop.core.Activator.getDefault().getImageRegistry().get("ICON_TINY_SQUARE");
			} else if (columnIndex == 1) {
				if (mapping.mappingType == 1) {
					return org.eclipse.vtp.desktop.core.Activator.getDefault().getImageRegistry().get("ICON_DOMAIN");
				}
			}
	...
	public class ValueDialog extends FramedDialog {
		...
		protected void createDialogContents(Composite parent) {
			...
			int columnSel = 0;
			List<IDatabase> databases = getElement().getDesign().getDocument().getProject().getDatabaseSet()
					.getDatabases();
