...
public final class SelectionRequestCommand extends ConversationCommand {
	...
	/**
	 * Returns the type of the output at the supplied output index for the option at
	 * the specified index.
	 * 
	 * @param optionIndex       The option index to check.
	 * @param optionOutputIndex The output index to check.
	 * @return The type of the output at the supplied output index for the option at
	 *         the specified index.
	 */
	...
	/**
	 * Sets the input descriptor to a resource at the specified path for the option
	 * at the specified index.
	 * 
	...
	public Object exportContents() {
		List<String> properties = new ArrayList<String>(this.properties.size() * 2);
		...
		List<Object> options = new ArrayList<Object>(this.options.size());
		for (Iterator<Option> i = this.options.iterator(); i.hasNext();) {
		...
		return new Object[] { selectionName, resultName, filledResultValue, noInputResultValue, noMatchResultValue,
				properties.toArray(new String[properties.size()]), output.toArray(new Object[output.size()]),
				options.toArray(), parameters.toArray(new String[parameters.size()]) };
	}
	...
	public static final class Option {
		...
		/**
		 * Configures the contents of this command with a structure previously returned
		 * from {@link #exportContents()}.
		 * 
