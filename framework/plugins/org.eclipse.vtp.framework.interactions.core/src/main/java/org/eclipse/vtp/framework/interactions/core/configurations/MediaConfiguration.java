...
public class MediaConfiguration implements IConfiguration, InteractionsConstants {
	...
	private final Map<String, OutputConfiguration> outputConfigurations = new LinkedHashMap<String, OutputConfiguration>();
	/** Index of output configurations by name. */
	private final Map<String, InputConfiguration> inputConfigurations = new LinkedHashMap<String, InputConfiguration>();
	/** Index of property configurations by name. */
	private final Map<String, PropertyConfiguration> propertyConfigurations = new LinkedHashMap<String, PropertyConfiguration>();
	...
	public String[] getOutputConfigurationNames() {
		return (String[]) outputConfigurations.keySet().toArray(new String[outputConfigurations.size()]);
	}
	 ...
	 * @param configuration The configuration to register or <code>null</code> to
	...
	public String[] getInputConfigurationNames() {
		return (String[]) inputConfigurations.keySet().toArray(new String[inputConfigurations.size()]);
	}
	...
	public String[] getPropertyConfigurationNames() {
		return (String[]) propertyConfigurations.keySet().toArray(new String[propertyConfigurations.size()]);
	}
	...
	public void save(Element configurationElement) {
		...
		for (Entry<String, OutputConfiguration> entry : outputConfigurations.entrySet()) {
			Element element = configurationElement.getOwnerDocument().createElementNS(NAMESPACE_URI, outputName);
		...
		for (Entry<String, InputConfiguration> entry : inputConfigurations.entrySet()) {
			Element element = configurationElement.getOwnerDocument().createElementNS(NAMESPACE_URI, inputName);
			((IConfiguration) entry.getValue()).save(element);
			element.setAttribute(NAME_NAME, (String) entry.getKey());
			configurationElement.appendChild(element);
		}
		for (Entry<String, PropertyConfiguration> entry : propertyConfigurations.entrySet()) {
			Element element = configurationElement.getOwnerDocument().createElementNS(NAMESPACE_URI, propertyName);
			((IConfiguration) entry.getValue()).save(element);
			element.setAttribute(NAME_NAME, (String) entry.getKey());
			configurationElement.appendChild(element);
		}
