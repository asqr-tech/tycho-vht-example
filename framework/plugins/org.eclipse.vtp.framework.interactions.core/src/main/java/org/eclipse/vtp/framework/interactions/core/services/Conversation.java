...
public class Conversation implements IConversation {
	...
	public Conversation(ICommandProcessor commandProcessor, IBrandSelection brandSelection,
			IInteractionTypeSelection interactionTypeSelection, ILanguageSelection languageSelection,
			IMediaProviderRegistry mediaProviderRegistry, IVariableRegistry variableRegistry,
			IScriptingService scriptingService, ILastResult lastResult, IExecutionContext context) {
	...
	public List<Content> resolveOutput(OutputConfiguration configuration) {
		...
		while (brand != null && mediaProviderID == null) {
			mediaProviderID = mediaProviderRegistry.lookupMediaProviderID(brand.getId(), interactionTypeID, languageID);
		...
		List<Content> content = new MediaRenderingManager(mediaProviderRegistry.getMediaProvider(mediaProviderID),
				dataSet).renderContent(MediaRenderingManager.COMPLETE, results);
		return content;
	}

	private void resolveContentNode(OutputNode node, List<Content> results) {
		if (node instanceof OutputContent)
			for (Content content : ((OutputContent) node).getContent())
				results.add(content);
		else if (node instanceof OutputSwitch) {
			for (OutputCase c : ((OutputSwitch) node).getCases()) {
				Object result = scriptingService.createScriptingEngine(c.getScriptingLanguage()).execute(c.getScript());
	...
	/**
	 * Resolves the value of a property configuration using the selected brand and
	 * interaction type.
	 * 
	 * @param configuration      The configuration to resolve.
	...
	public String resolveProperty(PropertyConfiguration configuration, boolean useInteractionType,
		...
		IBrand brand = brandSelection.getSelectedBrand();
		while (brand != null) {
			value = configuration.getItem(brand.getName(), useInteractionType ? interactionTypeID : "", //$NON-NLS-1$
		...
		if (PropertyConfiguration.VARIABLE.equals(value.getType())) {
			...
			result = String.valueOf(scriptingService.createScriptingEngine("JavaScript").execute(value.getValue()));
		} else {
	 ...
	 */
	private List<MetaDataItemConfiguration> resolveMetaData(MetaDataConfiguration configuration) {
		String interactionTypeID = interactionTypeSelection.getSelectedInteractionType().getId();
		String languageID = languageSelection.getSelectedLanguage();
		MetaDataItemConfiguration[] items = null;
		IBrand brand = brandSelection.getSelectedBrand();
		for (; brand != null && items == null; brand = brand.getParentBrand())
			items = configuration.getItem(brand.getName() + interactionTypeID + languageID);
		if (items == null) {
		...
		List<MetaDataItemConfiguration> results = new ArrayList<MetaDataItemConfiguration>(items.length);
		for (int i = 0; i < items.length; ++i) {
			...
			case MetaDataItemConfiguration.TYPE_EXPRESSION:
				...
				item.setStaticValue(String.valueOf(scriptingService
						.createScriptingEngine(items[i].getScriptingLanguage()).execute(items[i].getValue())));
				break;
			case MetaDataItemConfiguration.TYPE_VARIABLE:
				item = new MetaDataItemConfiguration();
				item.setName(items[i].getName());
				item.setStaticValue(dataSet.getData(items[i].getValue()).toString());
				break;
	...
	public IEndMessage createEndMessage(AssignmentConfiguration[] configurations) {
		...
		for (int i = 0; i < configurations.length; ++i)
			message.setVariableValue(configurations[i].getName(),
					variableRegistry.getVariable(configurations[i].getName()).toString());
		return message;
	}
	...
	private final class Initial extends AbstractInteraction implements IInitial {
		 ...
		 * 
		 * @param resultParameterName The name of the parameter to set the result value
		 *                            to.
		 */
		...
		ConversationCommand createCommand() {
			...
			if (mediaConfig != null) {
				...
				for (int i = 0; i < propertyNames.length; ++i) {
					String value = resolveProperty(mediaConfig.getPropertyConfiguration(propertyNames[i]), true);
					if (value != null)
				...
				for (Iterator<?> i = content.iterator(); i.hasNext();) {
					...
					if (item instanceof TextContent) {
						...
						command.addOutput(resolveFilePath(outputConfiguration, ((FileContent) item).getPath()));
				}
			}
	...
	private final class MetaDataMessage extends AbstractInteraction implements IMetaDataMessage {
		...
		ConversationCommand createCommand() {
			...
			command.setResultName(resultParameterName);
			command.setFilledResultValue(RESULT_NAME_FILLED);
			List<MetaDataItemConfiguration> metaData = resolveMetaData(configuration);
			for (Iterator<MetaDataItemConfiguration> i = metaData.iterator(); i.hasNext();) {
	...
	private final class InputRequest extends AbstractInteraction implements IInputRequest {
		...
		ConversationCommand createCommand() {
			...
			if (mediaConfig != null) {
				...
				InputConfiguration inputConfiguration = mediaConfig.getInputConfiguration(configuration.getInputName());
				InputGrammar grammar = resolveInput(inputConfiguration);
				if (grammar instanceof FileInputGrammar)
					command.setInput(resolveFilePath(inputConfiguration, ((FileInputGrammar) grammar).getPath()));
				...
				if (grammar2 instanceof FileInputGrammar)
					command.setInput2(resolveFilePath(inputConfiguration2, ((FileInputGrammar) grammar2).getPath()));
	...
	private final class SelectionRequest extends AbstractInteraction implements ISelectionRequest {
		...
		ConversationCommand createCommand() {
			...
			Map<String, SelectionChoiceConfiguration> choiceIndex = new HashMap<String, SelectionChoiceConfiguration>(
					choices.length);
			...
			if (choiceNames != null) {
				for (int i = 0, index = 0; i < choiceNames.length; ++i) {
					...
					if (scriptingLanguage != null && script != null) {
						IScriptingEngine engine = scriptingService.createScriptingEngine(scriptingLanguage);
						if (engine != null) {
							Object scriptResult = engine.execute(script);
							if (Boolean.FALSE.toString().equalsIgnoreCase(String.valueOf(scriptResult)))
								continue;
						}
					}
					...
					if (mediaConfig != null) {
						...
						for (int j = 0; j < propertyNames.length; ++j) {
							String value = resolveProperty(mediaConfig.getPropertyConfiguration(propertyNames[j]),
									true);
							if (value != null)
						...
						for (Iterator<?> j = content.iterator(); j.hasNext();) {
							...
							if (item instanceof TextContent) {
								...
								command.addOptionOutput(index,
										resolveFilePath(outputConfiguration, ((FileContent) item).getPath()));
						}
						...
						if (grammar instanceof FileInputGrammar)
							command.setOptionInput(index,
									resolveFilePath(inputConfiguration, ((FileInputGrammar) grammar).getPath()));
						else if (grammar instanceof BuiltInInputGrammar) {
							Input customInput = new Input(ConversationCommand.INPUT_TYPE_CUSTOM);
							customInput.setProperty("value", ((BuiltInInputGrammar) grammar).getBuiltInInputURI());
	...
	private final class DataRequest extends AbstractInteraction implements IDataRequest {
		...
		ConversationCommand createCommand() {
			...
			if (mediaConfig != null) {
				...
				InputGrammar grammar = resolveInput(mediaConfig.getInputConfiguration(configuration.getInputName()));
				if (grammar instanceof FileInputGrammar)
					command.setInput(resolveFilePath(mediaConfig.getInputConfiguration(configuration.getInputName()),
		...
		ConversationCommand createCommand() {
			...
			for (int i = 0; i < keys.length; ++i) {
				if (!configuration.isInputVariable(keys[i])) // this determines
					...
					IDataObject obj = variableRegistry.getVariable(configuration.getInputValue(keys[i]));
					if (obj != null)
			...
			for (int i = 0; i < keys.length; ++i)
				command.setOutputArgumentValue(keys[i], configuration.getOutputValue(keys[i]));
			...
			for (int i = 0; i < keys.length; ++i) {
				if (!configuration.isURLParameterVariable(keys[i])) // this
					...
					IDataObject obj = variableRegistry.getVariable(configuration.getURLParameterValue(keys[i]));
					if (obj != null)
	...
	private final class BridgeMessage extends AbstractInteraction implements IBridgeMessage {
		...
		ConversationCommand createCommand() {
			...
			if (bridgedValue == null)
				bridgedValue = resolveProperty(transferTypePropertyConfig, true, false);
		...
		public Object getData(String name) {
			if (name.startsWith("LastResult")) {
				if (name.length() > "LastResult.".length()) {
					...
					ILastResultData data = (ILastResultData) lastResult.getResults().get(0);
					if (data == null)
						return null;
