 ...
 * @author Lonnie Pryor
 */
public class InputConfiguration implements IConfiguration, InteractionsConstants {
	...
	private final Map<String, InputGrammar> items = new HashMap<String, InputGrammar>();
	 ...
	 * @param interactionTypeID The ID of the interaction type to find the item for.
	 * @param languageID        The ID of the language to find the item for.
	...
	public InputGrammar getItem(String brandName, String interactionTypeID, String languageID) {
		return (InputGrammar) items.get(brandName + interactionTypeID + languageID);
	}
	 ...
	 * @param interactionTypeID The ID of the interaction type to set the item for.
	 * @param languageID        The ID of the language to set the item for.
	 * @param item              The item to set as the configuration or
	 *                          <code>null</code> to remove the specified
	 *                          configuration.
	 */
		...
		for (int i = 0; i < itemElements.getLength(); ++i) {
			...
			for (int j = 0; input == null && j < inputElements.getLength(); j++)
				if (inputElements.item(j) instanceof Element)
					input = inputFactory.loadInput((Element) inputElements.item(j));
			if (input != null)
				items.put(key, input);
		}
	}
	...
	public void save(Element configurationElement) {
		...
		for (Entry<String, InputGrammar> entry : items.entrySet()) {
