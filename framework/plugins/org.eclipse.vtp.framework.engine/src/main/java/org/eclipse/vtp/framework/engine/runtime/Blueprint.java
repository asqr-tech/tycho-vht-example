...
public class Blueprint {
	...
	public Blueprint(IProcessDefinition definition, ConfigurationDescriptor[] configurationDescriptors,
			ActionDescriptor[] actionDescriptors, ObserverDescriptor[] observerDescriptors,
		...
		List<ServiceDescriptor> actionServiceDescriptors = new ArrayList<ServiceDescriptor>(serviceDescriptors.length);
		for (int i = 0; i < serviceDescriptors.length; ++i) {
			if (ServiceDescriptor.SCOPE_PROCESS.equals(serviceDescriptors[i].getScope()))
				processServiceDescriptors.add(serviceDescriptors[i]);
			else if (ServiceDescriptor.SCOPE_SESSION.equals(serviceDescriptors[i].getScope()))
				sessionServiceDescriptors.add(serviceDescriptors[i]);
			else if (ServiceDescriptor.SCOPE_EXECUTION.equals(serviceDescriptors[i].getScope()))
				executionServiceDescriptors.add(serviceDescriptors[i]);
			else if (ServiceDescriptor.SCOPE_ACTION.equals(serviceDescriptors[i].getScope()))
				actionServiceDescriptors.add(serviceDescriptors[i]);
		}
		this.processServiceIndex = buildServices(definition, processServiceDescriptors);
		this.sessionServiceIndex = buildServices(definition, sessionServiceDescriptors);
		this.executionServiceIndex = buildServices(definition, executionServiceDescriptors);
		this.actionServiceIndex = buildServices(definition, actionServiceDescriptors);
		this.executableIndex = buildExecutables(definition, actionDescriptors, observerDescriptors);
	...
	public Collection<Configuration> createConfigurations(Element data) {
		...
		List<Configuration> results = new ArrayList<Configuration>(descriptors.size());
	 ...
	 */
	private Map<String, LinkedList> indexConfigurations(ConfigurationDescriptor[] configurationDescriptors) {
		Map<String, LinkedList> configurationDescriptorIndex = new HashMap<String, LinkedList>(
				configurationDescriptors.length);
		for (int i = 0; i < configurationDescriptors.length; ++i) {
			String xmlIdentifier = RuntimeUtils.getQualifiedIdentifier(configurationDescriptors[i].getXmlTag(),
					configurationDescriptors[i].getXmlNamespace());
			LinkedList<ConfigurationDescriptor> list = (LinkedList<ConfigurationDescriptor>) configurationDescriptorIndex
					.get(xmlIdentifier);
			if (list == null)
				configurationDescriptorIndex.put(xmlIdentifier, list = new LinkedList<ConfigurationDescriptor>());
			list.addLast(configurationDescriptors[i]);
		}
		for (Iterator<?> i = configurationDescriptorIndex.entrySet().iterator(); i.hasNext();) {
		...
		return Collections.unmodifiableMap(new HashMap<String, LinkedList>(configurationDescriptorIndex));
	}
	...
	private Map<String, Object> buildServices(IProcessDefinition definition,
		...
		for (Iterator<ServiceDescriptor> i = serviceDescriptors.iterator(); i.hasNext();) {
			...
			for (Iterator<?> j = service.getIdentifiers().iterator(); j.hasNext();) {
				...
				if (list == null)
					serviceIndex.put(identifier, list = new LinkedList<Service>());
	 ...
	 * @return A Map of service lists indexed by identifiers.
	 */
	private Map<String, Object> buildExecutables(IProcessDefinition definition, ActionDescriptor[] actionDescriptors,
		...
		for (int i = 0; i < actionIDs.length; ++i) {
			...
			Action action = new Action(this, definition.getActionName(actionID),
					...
					((ActionDescriptor) actionIndex.get(definition.getActionDescriptorID(actionID))));
			...
			for (int j = 0; j < observerIDs.length; ++j) {
				...
				Observer observer = new Observer(this, definition.getObserverConfiguration(observerID), observerID,
						((ObserverDescriptor) observerIndex.get(definition.getObserverDescriptorID(observerID))),
						action);
			...
			for (int j = 0; j < resultIDs.length; ++j) {
				observerIDs = definition.getAfterObserverInstanceIDs(actionID, resultIDs[j]);
	...
	/**
	 * Links the flow of the entire process and returns the entry point executable.
	 * 
	 * @param definition The process definition.
	...
	private Map<String, Executable> linkExecutables(IProcessDefinition definition) {
		...
		for (int i = 0; i < actionIDs.length; ++i) {
			...
			for (int j = 0; j < resultIDs.length; ++j) {
				...
				Executable target = (Executable) targetsByActionID
						.get(definition.getActionResultTargetInstanceID(actionIDs[i], resultIDs[j]));
				if (previous == null)
					result = target;
				else
		...
		for (String startActionId : startActionIds) {
			System.out.println(
					"Indexing start action: " + definition.getActionName(startActionId) + "[" + startActionId + "]");
