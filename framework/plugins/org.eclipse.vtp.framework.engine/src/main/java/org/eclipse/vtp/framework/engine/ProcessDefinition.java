...
public class ProcessDefinition implements IProcessDefinition {
	...
	private final Map<String, List<Node>> serviceConfigurations = new HashMap<String, List<Node>>();
	/** The names of the actions. */
	private final Map<String, String> actionNames = new HashMap<String, String>();
	/** The sequence definitions. */
	private final Map<String, SequenceDefinition> sequenceDefinitions = new HashMap<String, SequenceDefinition>();
	/** The observer descriptor IDs by instance ID. */
	private final Map<String, String> observerDescriptorIDs = new HashMap<String, String>();
	/** The observer configurations by instance ID. */
	private final Map<String, List<Node>> observerConfigurations = new HashMap<String, List<Node>>();
	 ...
	 * @throws NullPointerException     If the specified document is
	 *                                  <code>null</code>.
	 */
	public ProcessDefinition(Document document) throws IllegalArgumentException, NullPointerException {
		...
		for (int i = 0; i < list.getLength(); ++i) {
			...
			List<Node> configs = new ArrayList<Node>(elements.getLength());
		...
		for (int i = 0; i < list.getLength(); ++i) {
			...
			SequenceDefinition sequence = new SequenceDefinition(id, action.getAttribute("descriptor-id"), //$NON-NLS-1$
		...
		for (int i = 0; i < list.getLength(); ++i) {
			...
			List<Node> configurations = new ArrayList<Node>(elements.getLength());
			for (int j = 0; j < elements.getLength(); ++j)
				if (elements.item(j) instanceof Element)
		...
		for (int i = 0; i < list.getLength(); ++i) {
			...
			sequence.beforeObservers = new ArrayList<String>(notify.getLength());
		...
		for (int i = 0; i < list.getLength(); ++i) {
			...
			if (sequence.targets.containsKey(path))
				throw new IllegalArgumentException();
			SequenceDefinition targetSequence = (SequenceDefinition) sequenceDefinitions.get(target);
			if (targetSequence == null)
				throw new IllegalArgumentException();
			sequence.targets.put(path, target);
			NodeList notify = after.getElementsByTagNameNS(URI, "notify"); //$NON-NLS-1$
			List<String> afterObservers = new ArrayList<String>(notify.getLength());
		...
		// Finish up
		List<String> startIDs = new ArrayList<String>(Arrays.asList(definition.getAttribute("start").split(",")));
	...
	public String[] getActionInstanceIDs() {
		return (String[]) sequenceDefinitions.keySet().toArray(new String[sequenceDefinitions.size()]);
	}
	...
	public String getActionDescriptorID(String actionInstanceID) {
		SequenceDefinition sequence = (SequenceDefinition) sequenceDefinitions.get(actionInstanceID);
		if (sequence == null)
			return null;
	...
	public Element[] getActionConfiguration(String actionInstanceID) {
		...
		if (sequence == null)
			return new Element[0];
		return (Element[]) sequence.configurations.toArray(new Element[sequence.configurations.size()]);
	}
	...
	public String[] getBeforeObserverInstanceIDs(String actionInstanceID) {
		...
		if (sequence.beforeObservers == null)
			return new String[0];
		return (String[]) sequence.beforeObservers.toArray(new String[sequence.beforeObservers.size()]);
	}
	...
	public final class SequenceDefinition {
		...
		final List<Node> configurations;
		final Map<String, String> targets = new HashMap<String, String>();
		final Map<String, List<String>> afterObservers = new HashMap<String, List<String>>();
		List<String> beforeObservers = null;
		...
		SequenceDefinition(String actionInstanceID, String actionDescriptorID, NodeList configurations) {
			...
			this.configurations = new ArrayList<Node>(configurations.getLength());
