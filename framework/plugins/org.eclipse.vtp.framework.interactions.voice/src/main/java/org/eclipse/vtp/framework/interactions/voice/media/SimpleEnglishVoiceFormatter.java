...
public class SimpleEnglishVoiceFormatter extends VoiceFormatter {
	 ...
	 * 
	 * @see
	 * org.eclipse.vtp.framework.interactions.core.media.IFormatter#formatDate(java.
	 * util.Date, java.lang.String,
	 * org.eclipse.vtp.framework.interactions.core.media.IResourceManager)
	 */
	public List formatDate(Date date, String formatDefinition, String formatOptions, IResourceManager resourceManager) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		return formatDate(cal, formatDefinition, formatOptions, resourceManager);
	}

	public List formatDate(Calendar cal, String formatDefinition, String formatOptions,
		...
		if (formatDefinition == null || formatDefinition.equals("")) {
			...
			if (calYear == nowYear && calDay == nowDay) {
				...
				if (options.contains("this")) {
					if (calHour < 12)
						ret.add(getAudioContent(resourceManager, "/DayOfWeek/", "this_morning", "this morning"));
					else if (calHour < 17)
						ret.add(getAudioContent(resourceManager, "/DayOfWeek/", "this_afternoon", "this afternoon"));
					else
						ret.add(getAudioContent(resourceManager, "/DayOfWeek/", "this_evening", "this evening"));
					// Format this (morning, etc...)
				} else {
					if (options.contains("today")) {
						ret.add(getAudioContent(resourceManager, "/DayOfWeek/", "today", "today"));
					} else {
						ret.add(getAudioContent(resourceManager, "/DayOfWeek/", days[cal.get(Calendar.DAY_OF_WEEK) - 1],
	 ...
	 * 
	 * @see
	 * org.eclipse.vtp.framework.interactions.core.media.IFormatter#formatMoney(java
	 * .math.BigDecimal, java.util.Currency, java.lang.String,
	 * org.eclipse.vtp.framework.interactions.core.media.IResourceManager)
	 */
	public List<Content> formatMoney(BigDecimal money, Currency currency, String formatDefinition, String formatOptions,
			IResourceManager resourceManager) {
		List<Content> ret = new ArrayList<Content>();
	...
	/**
	 * Internal function to produce the transformation of a number in the hundreds
	 * range. The results of this function are appended to the end of the given list
	 * of content.
	 * 
	...
	private void translateNumberInHundreds(IResourceManager resourceManager, int number, List<Content> content) {
		...
		if (hundreds > 0) {
			content.add(
					getAudioContent(resourceManager, "/Digits/", String.valueOf(hundreds), String.valueOf(hundreds)));
	...
	/**
	 * Internal function to transform the given string into the elements used in the
	 * English language to describe a year.
	 * 
	 ...
	 * getDefaultFormats(org.eclipse.vtp.framework.interactions.core.media.
	 * FormattableContent)
	 */
